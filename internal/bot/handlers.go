...
func (b *Bot) formatDiary(user *UserState, diary *eljur.DiaryResponse) error {
        if len(diary.Response.Result.Students) == 0 {
                return b.SendMessage(user.ChatID, "📚 Дневник пуст для выбранной недели", nil)
        }

        student := diary.Response.Result.Students[0]
        text := "📚 *Дневник*\n\n"
        ...
        for _, day := range student.Days {
                ...
                for _, lesson := range day.Lessons {
                        text += fmt.Sprintf("   %d. *%s*", lesson.Number, lesson.Name)
